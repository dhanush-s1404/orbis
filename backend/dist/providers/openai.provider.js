...
class OpenAIProvider {
    ...
    async generateContent(profile, templateSections) {
        ...
        try {
            ...
            const validation = ai_service_1.AIService.validateAIOutput(parsed);
            if (!validation.valid) {
                ...
                return {
                    ...
                    metadata: {
                        generationId: `gen-${Date.now()}`,
                        templateId: "",
                        sectionsGenerated: (parsed.sections || []).map((s) => s.type || "unknown"),
                        generationTime: 0,
                        modelUsed: this.model,
            ...
            return {
                success: false,
                errors: [`Failed to parse OpenAI response: ${e.message}`],
                metadata: {
                    generationId: `gen-${Date.now()}`,
                    templateId: "",
                    sectionsGenerated: [],
    ...
    async rewriteContent(existingContent, instruction) {
        const response = await this.client.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are an ORBIS website builder AI. Rewrite the given content according to the instruction. Return only the rewritten text, no explanations, no JSON, no additional commentary.",
                },
                {
                    role: "user",
    ...
    buildSystemPrompt(profile, templateSections) {
        const sectionDescriptions = {
            hero: "Hero section with title, subtitle, primary CTA, secondary CTA, background styling",
            features: "Features section with feature titles, descriptions, and relevant labels",
            about: "About section with business story, mission statement, supporting description",
            services: "Services section with service names and descriptions",
            testimonials: "Testimonials section with customer names, roles, and content",
            pricing: "Pricing section with plan names, prices, and feature lists",
            cta: "CTA section with heading, supporting text, and button label",
            contact: "Contact section with contact information, form fields, or address",
            footer: "Footer section with copyright, navigation links, and social links",
        };
